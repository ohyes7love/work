...
public class GenGuidService {
	...
	/**
	 * 랜덤 값을 처리하기 위해 정의한 RANDOM 변수 ThreadLocalRandom은 동시성 문제를 해결하기 위해 각 쓰레드마다 생성된
	 * 인스턴스에서 각각 난수를 반환한다. 따라서 Random과 같은 경합 문제가 발생하지 않아 안전하며, 성능상 이점이 있어 사용
	 */
...
//	private RedisTemplate<String, String> redisTemplateString;
	...
	/**
	 * guid를 생성한다. GUID생성룰: yyyyMMddHHmmssSSS(17) + 시스템코드(3) + 노드번호(2) + 인스턴스번호(2) +
	 * 랜덤String(6) = 30자리
	 * <p>
	 * 생성룰에 따라 guid를 생성 후 생성 문자열을 리턴한다. 중복검증: redis에 guid를 key로 입력하여 redis에 데이터가
	 * 존재하면 중복발생으로 오류를 처리한다. redis에 입력된 데이터는 TTL이 1초로 설정되어 1초 뒤 삭제된다.
	 * <p>
	 *
	 * @param GuidInDto String sendChlCd : guid채번요청 시스템코드 Integer sendSysNodeNo :
	 *                  guid채번요청 시스템의 노드번호 Integer sendSysInstNo : guid채번요청 시스템의
	 *                  인스턴스번호
	...
	public String generateGuid(GuidInDto in) {
		...
		append(guidBuffer, String.format("%02d", in.getSendSysNodeNo())); // guidBuffer에 guid채번요청 시스템 노드번호를 두 자리로 변환하여
																			// 추가하여 guid 생성
		append(guidBuffer, String.format("%02d", in.getSendSysInstNo())); // guidBuffer에 guid채번요청 시스템 인스턴스번호를 두 자리로 변환하여
																			// 추가하여 guid 생성
...
//	    }
	...
	public Mono<GuidOutDto> generateGuidMono(GuidInDto in) {
		// guid 생성 로직
		StringBuffer guidBuffer = new StringBuffer(30); // StringBuffer를 사용하여 guidBuffer 생성
		append(guidBuffer, DateUtils.getCurrentDataTimeMillis()); // guidBuffer에 현재 시간(yyyyMMddHHmmssSSS)을 추가하여 guid 생성
		append(guidBuffer, in.getSendChlCd()); // guidBuffer에 guid채번요청 시스템코드를 추가하여 guid 생성
		...
		;
		GuidOutDto guidOutDto = new GuidOutDto();
		guidOutDto.setGuid(guid);
		return Mono.just(guidOutDto);
	}
